...
```{r include=TRUE}
# creating a list storing spatrasters
list = list(rcl_sst, rcl_depth)
...
oyster_zones <- extract(oyster_homes, wc_regions_clean)

# logical vector out of list
oyster_zones_vector <- unlist(oyster_zones)

# visualize
plot(oyster_zones_vector)
```
...
eez <- rasterize(wc_regions_clean, oyster_homes)

# calculate total suitable area in rgns
tot_suitable_area <- zonal(cell_area,
                           eez,
                           fun = sum,
                           na.rm = TRUE)
```
...
```{r include=TRUE}
# percentage suitable
suitable_percentage <- tot_suitable_area / st_area(wc_regions_clean) * 100
